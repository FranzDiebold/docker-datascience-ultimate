...
build-server-arm:  ## Build server Docker image (ARM architecture).
...
build-server-amd:  ## Build server Docker image (X86 architecture).
	docker buildx build -t franzdiebold/datascience-ultimate-server:latest --platform=linux/amd64 datascience-ultimate-server/

.PHONY: build-lab-arm
build-lab-arm:  ## Build JupyterLab Docker image (ARM architecture).
	docker buildx build -t franzdiebold/datascience-ultimate:latest --platform=linux/arm64 datascience-ultimate/

.PHONY: build-lab-amd
build-lab-amd:  ## Build JupyterLab Docker image (X86 architecture).
	docker buildx build -t franzdiebold/datascience-ultimate:latest --platform=linux/amd64 datascience-ultimate/

.PHONY: build-arm
build-arm: build-server-arm build-lab-arm  ## Build all Docker images (ARM architecture).

.PHONY: build-amd
build-amd: build-server-amd build-lab-amd  ## Build all Docker images (X86 architecture).
